...
ds_l_w  = [ 10,  20,   50,  100, 1000];
ds_l_dB = [-26, -12, -3.5, -1.5,    0];
ds_h_w  = [ 1000, 5000, 10000, 20000];
...
[ds_h_B, ds_h_A] = invfreqz(arrayfun(@db2mag, ds_h_dB), (ds_h_w.*((2*pi)/Fs)) , 2, 2);
...
clf;
semilogx(iec_w, ds_dB, 'g;ICS-43434 Datasheet plot (approx.);');
hold on;
...
semilogx(iec_w, iec_c1_t_dB, '--r;IEC 61672-1:2013 Class 1 tolerance;');
semilogx(iec_w, iec_c1_b_dB, '--r');
semilogx(iec_w, iec_c2_t_dB, 'r;IEC 61672-1:2013 Class 2 tolerance;');
semilogx(iec_w, iec_c2_b_dB, 'r');

...
struct SOS_IIR_Filter
{
    ...
    const int num_sos;
    const float gain;
    SOS_Coefficients *sos = nullptr;
    SOS_Delay_State *w = nullptr;

    // Dynamic constructor
    SOS_IIR_Filter(size_t num_sos, const float gain, const SOS_Coefficients _sos[] = nullptr)
        : num_sos(num_sos), gain(gain)
    {
        if (num_sos > 0)
        {
            sos = new SOS_Coefficients[num_sos];
            if ((sos != nullptr) && (_sos != nullptr))
   ...
   */
    ...
    {
        if ((num_sos < 1) || (sos == nullptr) || (w == nullptr))
            return 0;
        float *source = input;
    ...
    /** 
   * Calculate sum of squares of all values in input array of floats, write values to output.
   */
    inline float sum_of_squares(float *input, float *output, size_t len)
    {
        if ((num_sos < 1) || (sos == nullptr) || (w == nullptr))
            return 0;
        // Apply last SOS with gain and return the sum of squares of all samples
        return sos_filter_sum_sqr_f32(input, output, len, sos[num_sos - 1], w[num_sos - 1], gain);
    }
